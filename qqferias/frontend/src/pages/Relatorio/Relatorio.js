    ...
    const fetchVacationRequests = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/qqferias/gestores/${decodedToken.user.id}/agendamentos-funcionarios`
        );
            ...
            if (!addedIds.has(currentId)) {
              ...
              if (funcionarioIndex !== -1) {
                const vacationRequest = {
                  ...
                  type: events.agendamentos[i]['antecipacao_13_salario']
                    ? 'Solicitação de férias com adiantamento do 13º'
